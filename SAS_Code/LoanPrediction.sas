...
Description: SAS Codes for Data Analytical Programming 
Program written by: Nikhil Mirpuri
Data first written: Mon, 05-June-2024
Data last updated: Fri, 05-June-2024
...
libname projlib '/home/u63554838/Final_Project/data/Personal Project/';
proc import out=projlib.trainingdata datafile='/home/u63554838/Final_Project/data/Personal Project/TRAINING_DS.csv' dbms=CSV replace; 
run;

proc import out=projlib.testingdata datafile='/home/u63554838/Final_Project/data/Personal Project/TESTING_DS.csv' dbms=CSV replace; 
run;


proc print data=projlib.mydata;
run;
proc freq data=projlib.mydata;
table _all_;
run;

proc means data=projlib.mydata;
run;
...
CREATE TABLE projlib.TRAINING_DS_TP061966_BK AS
SELECT * FROM projlib.trainingdata
;
...
DESCRIBE TABLE projlib.TRAINING_DS_TP061966_BK;
...
DATA projlib.TRAINING_DS_TP061966_BK;
SET projlib.TRAINING_DS_TP061966_BK;
...
SELECT * FROM projlib.TRAINING_DS_TP061966_BK;
...
PROC FREQ DATA = projlib.TRAINING_DS_TP061966_BK;
TABLE GENDER;
RUN;
ODS GRAPHICS / RESET WIDTH=4.0 IN HEIGHT=3.0 IN IMAGEMAP;
PROC SGPLOT DATA = projlib.TRAINING_DS_TP061966_BK;
VBAR GENDER;
RUN;
...
PROC MEANS DATA = projlib.TRAINING_DS_TP061966_BK N NMISS MIN MAX MEAN MEDIAN STD;
...
%MACRO_BIVA_CV_TP061966(projlib.TRAINING_DS_TP061966_BK, GENDER, MARITAL_STATUS, 'Bivariate analysis', 'on GENDER (Categorical) Vs. MARITAL_STATUS (Categorical)');


/* FAMILY_MEMBERS Vs. QUALIFICATION */
%MACRO_BIVA_CV_TP061966(projlib.TRAINING_DS_TP061966_BK, FAMILY_MEMBERS, QUALIFICATION, 'Bivariate analysis', 'on FAMILY_MEMBERS (Categorical) Vs. QUALIFICATION (Categorical)');


/* EMPLOYMENT Vs. LOAN_HISTORY */
%MACRO_BIVA_CV_TP061966(projlib.TRAINING_DS_TP061966_BK, EMPLOYMENT, LOAN_HISTORY, 'Bivariate analysis', 'on EMPLOYMENT (Categorical) Vs. LOAN_HISTORY (Categorical)');


/* LOAN_LOCATION Vs. LOAN_APPROVAL_STATUS */
%MACRO_BIVA_CV_TP061966(projlib.TRAINING_DS_TP061966_BK, LOAN_LOCATION, LOAN_APPROVAL_STATUS, 'Bivariate analysis', 'on LOAN_LOCATION (Categorical) Vs. LOAN_APPROVAL_STATUS (Categorical)');


/* GENDER Vs. LOAN_APPROVAL_STATUS */
%MACRO_BIVA_CV_TP061966(projlib.TRAINING_DS_TP061966_BK, GENDER, LOAN_APPROVAL_STATUS, 'Bivariate analysis', 'on GENDER (Categorical) Vs. LOAN_APPROVAL_STATUS (Categorical)');
...
PROC MEANS DATA = projlib.TRAINING_DS_TP061966_BK;
...
PROC CORR DATA = projlib.TRAINING_DS_TP061966_BK PLOTS = SCATTER;
...
/************************************************************************
Imputation for categorical variable on dataset projlib.TRAINING_TP061966
*************************************************************************/
/******************************
...
CREATE TABLE projlib.TRAINING_FI_GENDER_DS_BK AS
SELECT * FROM projlib.TRAINING_DS_TP061966_BK;
...
PROC SQL;
SELECT *
FROM projlib.TRAINING_FI_GENDER_DS_BK g
...
CREATE TABLE projlib.TRAINING_FI_GENDER_DS_BK_NEW AS
...
SELECT * 
FROM projlib.TRAINING_FI_GENDER_DS_BK_NEW;
QUIT;
...
FROM projlib.TRAINING_FI_GENDER_DS_BK_NEW g;
QUIT;
...
FROM projlib.TRAINING_FI_GENDER_DS_BK_NEW go
WHERE go.TOTAL_COUNTS EQ ( SELECT MAX(gi.TOTAL_COUNTS)
						   FROM projlib.TRAINING_FI_GENDER_DS_BK_NEW gi); 
...
PROC SQL;
UPDATE projlib.TRAINING_FI_GENDER_DS_BK
...
*******************************/
/* Step 1. Make a copy of projlib.TRAINING_FI_GENDER_DS_BK */

PROC SQL;

CREATE TABLE projlib.TRAINING_FI_FAMILY_MEMBERS_DS_BK AS
SELECT * FROM projlib.TRAINING_FI_GENDER_DS_BK;
...
PROC SQL;
SELECT *
FROM projlib.TRAINING_FI_FAMILY_MEMBERS_DS_BK fm
...
CREATE TABLE projlib.TRAINING_FI_FM_DS_BK_N AS
...
SELECT * 
FROM projlib.TRAINING_FI_FM_DS_BK_N;
QUIT;
...
FROM projlib.TRAINING_FI_FM_DS_BK_N fm;
QUIT;
...
FROM projlib.TRAINING_FI_FM_DS_BK_N fmo
WHERE fmo.TOTAL_COUNTS EQ ( SELECT MAX(fmi.TOTAL_COUNTS)
						   FROM projlib.TRAINING_FI_FM_DS_BK_N fmi); 
...
TITLE 'Impute the missing values using mode';
PROC SQL;
UPDATE projlib.TRAINING_FI_FAMILY_MEMBERS_DS_BK
...
*******************************/
/* Step 1. Make a copy of projlib.TRAINING_FI_FAMILY_MEMBERS_DS_BK */

PROC SQL;

CREATE TABLE projlib.TRAINING_FI_MARITAL_STATUS_DS_BK AS
SELECT * FROM projlib.TRAINING_FI_FAMILY_MEMBERS_DS_BK;
...
PROC SQL;
SELECT *
FROM projlib.TRAINING_FI_MARITAL_STATUS_DS_BK ms
...
CREATE TABLE projlib.TRAINING_FI_MS_DS_BK_NEW AS
...
SELECT * 
FROM projlib.TRAINING_FI_MS_DS_BK_NEW;
QUIT;
...
FROM projlib.TRAINING_FI_MS_DS_BK_NEW ms;
QUIT;
...
FROM projlib.TRAINING_FI_MS_DS_BK_NEW mso
WHERE mso.TOTAL_COUNTS EQ ( SELECT MAX(msi.TOTAL_COUNTS)
						   FROM projlib.TRAINING_FI_MS_DS_BK_NEW msi); 
...
TITLE 'Impute the missing values using mode';
PROC SQL;
UPDATE projlib.TRAINING_FI_MARITAL_STATUS_DS_BK
...
*******************************/
/* Step 1. Make a copy of projlib.TRAINING_FI_MARITAL_STATUS_DS_BK */

PROC SQL;

CREATE TABLE projlib.TRAINING_FI_EMPLOYMENT_DS_BK AS
SELECT * FROM projlib.TRAINING_FI_MARITAL_STATUS_DS_BK;
...
PROC SQL;
SELECT *
FROM projlib.TRAINING_FI_EMPLOYMENT_DS_BK e
...
CREATE TABLE projlib.TRAINING_FI_EMPLOYMENT_DS_BK_NEW AS
...
SELECT * 
FROM projlib.TRAINING_FI_EMPLOYMENT_DS_BK_NEW;
QUIT;
...
FROM projlib.TRAINING_FI_EMPLOYMENT_DS_BK_NEW e;
QUIT;
...
FROM projlib.TRAINING_FI_EMPLOYMENT_DS_BK_NEW eo
WHERE eo.TOTAL_COUNTS EQ ( SELECT MAX(ei.TOTAL_COUNTS)
						   FROM projlib.TRAINING_FI_EMPLOYMENT_DS_BK_NEW ei); 
...
TITLE 'Impute the missing values using mode';
PROC SQL;
UPDATE projlib.TRAINING_FI_EMPLOYMENT_DS_BK
...
*******************************/
/* Step 1. Make a copy of projlib.TRAINING_FI_EMPLOYMENT_DS_BK */

PROC SQL;

CREATE TABLE projlib.TRAINING_FI_LOAN_HISTORY_DS_BK AS
SELECT * FROM projlib.TRAINING_FI_EMPLOYMENT_DS_BK;
...
PROC SQL;
SELECT *
FROM projlib.TRAINING_FI_LOAN_HISTORY_DS_BK lh
...
CREATE TABLE projlib.TRAINING_FI_LH_DS_BK_NEW AS
...
SELECT * 
FROM projlib.TRAINING_FI_LH_DS_BK_NEW;
QUIT;
...
FROM projlib.TRAINING_FI_LH_DS_BK_NEW lh;
QUIT;
...
FROM projlib.TRAINING_FI_LH_DS_BK_NEW lho
WHERE lho.TOTAL_COUNTS EQ ( SELECT MAX(lhi.TOTAL_COUNTS)
						   FROM projlib.TRAINING_FI_LH_DS_BK_NEW lhi); 
...
TITLE 'Impute the missing values using mode';
PROC SQL;
UPDATE projlib.TRAINING_FI_LOAN_HISTORY_DS_BK
...
/************************************************************************
Imputation for continuous variable on dataset projlib.TRAINING_TP061966
*************************************************************************/
/******************************
...
PROC SQL;
SELECT * 
FROM projlib.TRAINING_FI_LOAN_HISTORY_DS_BK la
...
PROC SQL;
CREATE TABLE projlib.TRAINING_FI_LOAN_AMOUNT_DS_BK AS
SELECT * FROM projlib.TRAINING_FI_LOAN_HISTORY_DS_BK;
QUIT;

/* Step 4. Impute the missing values using mean for "LOAN_AMOUNT" variable */
TITLE 'Impute the missing values using mean';
PROC STDIZE DATA = projlib.TRAINING_FI_LOAN_AMOUNT_DS_BK REPONLY
METHOD = MEAN OUT=projlib.TRAINING_FI_LOAN_AMOUNT_DS_BK;
VAR LOAN_AMOUNT;
QUIT;
...
PROC SQL;
SELECT * 
FROM projlib.TRAINING_FI_LOAN_AMOUNT_DS_BK ld
...
PROC SQL;
CREATE TABLE projlib.TRAINING_FI_LOAN_DURATION_DS_BK AS
SELECT * FROM projlib.TRAINING_FI_LOAN_AMOUNT_DS_BK;
QUIT;

/* Step 4. Impute the missing values using mean for "LOAN_DURATION" variable */
TITLE 'Impute the missing values using mean';
PROC STDIZE DATA = projlib.TRAINING_FI_LOAN_DURATION_DS_BK REPONLY
METHOD = MEAN OUT=projlib.TRAINING_FI_LOAN_DURATION_DS_BK;
VAR LOAN_DURATION;
QUIT;
...
CREATE TABLE projlib.TESTING_DS_TP061966_BK AS
SELECT * FROM projlib.testingdata;
...
/* GENDER */
%MACRO_UVA_CatV_TP061966_TESTING(projlib.TESTING_DS_TP061966_BK, GENDER, 'Univariate analysis', 'on GENDER (Categorical) Variable');

/* MARITAL_STATUS */
%MACRO_UVA_CatV_TP061966_TESTING(projlib.TESTING_DS_TP061966_BK, MARITAL_STATUS, 'Univariate analysis', 'on MARITAL_STATUS (Categorical) Variable');

/* FAMILY_MEMBERS */
%MACRO_UVA_CatV_TP061966_TESTING(projlib.TESTING_DS_TP061966_BK, FAMILY_MEMBERS, 'Univariate analysis', 'on FAMILY_MEMBERS (Categorical) Variable');

/* QUALIFICATION */
%MACRO_UVA_CatV_TP061966_TESTING(projlib.TESTING_DS_TP061966_BK, QUALIFICATION, 'Univariate analysis', 'on QUALIFICATION (Categorical) Variable');

/* EMPLOYMENT */
%MACRO_UVA_CatV_TP061966_TESTING(projlib.TESTING_DS_TP061966_BK, EMPLOYMENT, 'Univariate analysis', 'on EMPLOYMENT (Categorical) Variable');

/* LOAN_HISTORY */
%MACRO_UVA_CatV_TP061966_TESTING(projlib.TESTING_DS_TP061966_BK, LOAN_HISTORY, 'Univariate analysis', 'on LOAN_HISTORY (Categorical) Variable');

/* LOAN_LOCATION */
%MACRO_UVA_CatV_TP061966_TESTING(projlib.TESTING_DS_TP061966_BK, LOAN_LOCATION, 'Univariate analysis', 'on LOAN_LOCATION (Categorical) Variable');
...
/* CANDIDATE_INCOME */
%MACRO_UVA_ConV_TP061966_TESTING(projlib.TESTING_DS_TP061966_BK, CANDIDATE_INCOME, 'Univariate analysis', 'on CANDIDATE_INCOME (Continuous) Variable');

/* GUARANTEE_INCOME */
%MACRO_UVA_ConV_TP061966_TESTING(projlib.TESTING_DS_TP061966_BK, GUARANTEE_INCOME, 'Univariate analysis', 'on GUARANTEE_INCOME (Continuous) Variable');

/* LOAN_AMOUNT */
%MACRO_UVA_ConV_TP061966_TESTING(projlib.TESTING_DS_TP061966_BK, LOAN_AMOUNT, 'Univariate analysis', 'on LOAN_AMOUNT (Continuous) Variable');

/* LOAN_DURATION */
%MACRO_UVA_ConV_TP061966_TESTING(projlib.TESTING_DS_TP061966_BK, LOAN_DURATION, 'Univariate analysis', 'on LOAN_DURATION (Continuous) Variable');
...
/* GENDER Vs. MARITAL_STATUS */
%MACRO_BIVA_Cat_Testing_TP061966(projlib.TESTING_DS_TP061966_BK, GENDER, MARITAL_STATUS, 'Bivariate analysis', 'on GENDER (Categorical) Vs. MARITAL_STATUS (Categorical)');


/* FAMILY_MEMBERS Vs. QUALIFICATION */
%MACRO_BIVA_Cat_Testing_TP061966(projlib.TESTING_DS_TP061966_BK, FAMILY_MEMBERS, QUALIFICATION, 'Bivariate analysis', 'on FAMILY_MEMBERS (Categorical) Vs. QUALIFICATION (Categorical)');


/* EMPLOYMENT Vs. LOAN_HISTORY */
%MACRO_BIVA_Cat_Testing_TP061966(projlib.TESTING_DS_TP061966_BK, EMPLOYMENT, LOAN_HISTORY, 'Bivariate analysis', 'on EMPLOYMENT (Categorical) Vs. LOAN_HISTORY (Categorical)');


/* GENDER Vs. QUALIFICATION */
%MACRO_BIVA_Cat_Testing_TP061966(projlib.TESTING_DS_TP061966_BK, GENDER, QUALIFICATION, 'Bivariate analysis', 'on GENDER (Categorical) Vs. QUALIFICATION (Categorical)');


/* GENDER Vs. LOAN_LOCATION */
%MACRO_BIVA_Cat_Testing_TP061966(projlib.TESTING_DS_TP061966_BK, GENDER, LOAN_LOCATION, 'Bivariate analysis', 'on GENDER (Categorical) Vs. LOAN_LOCATION (Categorical)');


/* FAMILY_MEMBERS Vs. LOAN_LOCATION */
%MACRO_BIVA_Cat_Testing_TP061966(projlib.TESTING_DS_TP061966_BK, FAMILY_MEMBERS, LOAN_LOCATION, 'Bivariate analysis', 'on FAMILY_MEMBERS (Categorical) Vs. LOAN_LOCATION (Categorical)');
...
%MACRO_BIVA_CatVCon_Test_TP061966(projlib.TESTING_DS_TP061966_BK, GENDER, CANDIDATE_INCOME, 'Bivariate analysis on GENDER ( CHAR ) Versus CANDIDATE_INCOME ( NUMERIC )');

/* MARITAL_STATUS Vs. GUARANTEE_INCOME */
%MACRO_BIVA_CatVCon_Test_TP061966(projlib.TESTING_DS_TP061966_BK, MARITAL_STATUS, GUARANTEE_INCOME, 'Bivariate analysis on MARITAL_STATUS ( CHAR ) Versus GUARANTEE_INCOME ( NUMERIC )');

/* EMPLOYMENT Vs. LOAN_AMOUNT */
%MACRO_BIVA_CatVCon_Test_TP061966(projlib.TESTING_DS_TP061966_BK, EMPLOYMENT, LOAN_AMOUNT, 'Bivariate analysis on EMPLOYMENT ( CHAR ) Versus LOAN_AMOUNT ( NUMERIC )');
...
/* CANDIDATE_INCOME Vs. GUARANTEE_INCOME */
%MACRO_BIVA_ConVCon_Test_TP061966(projlib.TESTING_DS_TP061966_BK, CANDIDATE_INCOME, GUARANTEE_INCOME, 'Bivariate analysis on CANDIDATE_INCOME ( NUMERIC ) Versus GUARANTEE_INCOME ( NUMERIC )');

/* LOAN_AMOUNT Vs. LOAN_DURATION */
%MACRO_BIVA_ConVCon_Test_TP061966(projlib.TESTING_DS_TP061966_BK, LOAN_AMOUNT, LOAN_DURATION, 'Bivariate analysis on LOAN_AMOUNT ( NUMERIC ) Versus LOAN_DURATION ( NUMERIC )');

/* CANDIDATE_INCOME Vs. LOAN_DURATION */
%MACRO_BIVA_ConVCon_Test_TP061966(projlib.TESTING_DS_TP061966_BK, CANDIDATE_INCOME, LOAN_DURATION, 'Bivariate analysis on CANDIDATE_INCOME ( NUMERIC ) Versus LOAN_DURATION ( NUMERIC )');


/************************************************************************
Imputation for categorical variable on dataset projlib.TESTING_TP061966
*************************************************************************/
/******************************
GENDER - Categorical Variable
*******************************/
...
CREATE TABLE projlib.TESTING_FI_GENDER_DS_BK AS
SELECT * FROM projlib.TESTING_DS_TP061966_BK;
...
PROC SQL;
SELECT *
FROM projlib.TESTING_FI_GENDER_DS_BK g
WHERE ( ( g.gender EQ '' ) OR
		( g.gender IS NULL) );
QUIT;
...
CREATE TABLE projlib.TESTING_FI_GENDER_DS_BK_NEW AS
SELECT g.GENDER AS GENDER_NAME, COUNT(*) AS TOTAL_COUNTS
FROM projlib.TESTING_FI_GENDER_DS_BK g
WHERE ( ( g.GENDER IS NOT NULL ) OR
		( g.GENDER NE '' ) )
...
SELECT * 
FROM projlib.TESTING_FI_GENDER_DS_BK_NEW;
QUIT;
...
PROC SQL;
SELECT MAX(g.TOTAL_COUNTS) AS MAX_COUNTS
FROM projlib.TESTING_FI_GENDER_DS_BK_NEW g;
QUIT;
...
PROC SQL;
SELECT go.GENDER_NAME
FROM projlib.TESTING_FI_GENDER_DS_BK_NEW go
WHERE go.TOTAL_COUNTS EQ ( SELECT MAX(gi.TOTAL_COUNTS)
						   FROM projlib.TESTING_FI_GENDER_DS_BK_NEW gi); 
						   /* subquery to find the mode for gender */
QUIT;
...
TITLE 'Impute the missing values using mode';
PROC SQL;
UPDATE projlib.TESTING_FI_GENDER_DS_BK
SET GENDER = ( SELECT go.GENDER_NAME
					FROM projlib.TESTING_FI_GENDER_DS_BK_NEW go 
					WHERE go.TOTAL_COUNTS EQ ( SELECT MAX(gi.TOTAL_COUNTS)
										 	   FROM projlib.TESTING_FI_GENDER_DS_BK_NEW gi ) )
...
FAMILY_MEMBERS - Categorical Variable
*******************************/
/* Step 1. Make a copy of projlib.TESTING_FI_GENDER_DS_BK */

PROC SQL;

CREATE TABLE projlib.TESTING_FI_FAMILY_MEMBERS_DS_BK AS
SELECT * FROM projlib.TESTING_FI_GENDER_DS_BK;
...
PROC SQL;
SELECT *
FROM projlib.TESTING_FI_FAMILY_MEMBERS_DS_BK fm
WHERE ( ( fm.FAMILY_MEMBERS EQ '' ) OR
		( fm.FAMILY_MEMBERS IS NULL) );
QUIT;
...
CREATE TABLE projlib.TESTING_FI_FM_DS_BK_N AS
SELECT fm.FAMILY_MEMBERS AS FAMILY_MEMBERS, COUNT(*) AS TOTAL_COUNTS
FROM projlib.TESTING_FI_FAMILY_MEMBERS_DS_BK fm
WHERE ( ( fm.FAMILY_MEMBERS IS NOT NULL ) OR
		( fm.FAMILY_MEMBERS NE '' ) )
...
SELECT * 
FROM projlib.TESTING_FI_FM_DS_BK_N;
QUIT;
...
PROC SQL;
SELECT MAX(fm.TOTAL_COUNTS) AS MAX_COUNTS
FROM projlib.TESTING_FI_FM_DS_BK_N fm;
QUIT;
...
PROC SQL;
SELECT fmo.FAMILY_MEMBERS
FROM projlib.TESTING_FI_FM_DS_BK_N fmo
WHERE fmo.TOTAL_COUNTS EQ ( SELECT MAX(fmi.TOTAL_COUNTS)
						   FROM projlib.TESTING_FI_FM_DS_BK_N fmi); 
						   /* subquery to find the mode for family members */
QUIT;
...
TITLE 'Impute the missing values using mode';
PROC SQL;
UPDATE projlib.TESTING_FI_FAMILY_MEMBERS_DS_BK
SET FAMILY_MEMBERS = ( SELECT fmo.FAMILY_MEMBERS
					FROM projlib.TESTING_FI_FM_DS_BK_N fmo
					WHERE fmo.TOTAL_COUNTS EQ ( SELECT MAX(fmi.TOTAL_COUNTS)
										 	   FROM projlib.TESTING_FI_FM_DS_BK_N fmi ) )
...
EMPLOYMENT - Categorical Variable
*******************************/
/* Step 1. Make a copy of projlib.TESTING_FI_FAMILY_MEMBERS_DS_BK */

PROC SQL;

CREATE TABLE projlib.TESTING_FI_EMPLOYMENT_DS_BK AS
SELECT * FROM projlib.TESTING_FI_FAMILY_MEMBERS_DS_BK;
...
PROC SQL;
SELECT *
FROM projlib.TESTING_FI_EMPLOYMENT_DS_BK e
WHERE ( ( e.EMPLOYMENT EQ '' ) OR
		( e.EMPLOYMENT IS NULL) );
QUIT;
...
CREATE TABLE projlib.TESTING_FI_EMPLOYMENT_DS_BK_NEW AS
SELECT e.EMPLOYMENT AS EMPLOYMENT, COUNT(*) AS TOTAL_COUNTS
FROM projlib.TESTING_FI_EMPLOYMENT_DS_BK e
WHERE ( ( e.EMPLOYMENT IS NOT NULL ) OR
		( e.EMPLOYMENT NE '' ) )
...
SELECT * 
FROM projlib.TESTING_FI_EMPLOYMENT_DS_BK_NEW;
QUIT;
...
PROC SQL;
SELECT MAX(e.TOTAL_COUNTS) AS MAX_COUNTS
FROM projlib.TESTING_FI_EMPLOYMENT_DS_BK_NEW e;
QUIT;
...
PROC SQL;
SELECT eo.EMPLOYMENT
FROM projlib.TESTING_FI_EMPLOYMENT_DS_BK_NEW eo
WHERE eo.TOTAL_COUNTS EQ ( SELECT MAX(ei.TOTAL_COUNTS)
						   FROM projlib.TESTING_FI_EMPLOYMENT_DS_BK_NEW ei); 
						   /* subquery to find the mode for employment */
QUIT;
...
TITLE 'Impute the missing values using mode';
PROC SQL;
UPDATE projlib.TESTING_FI_EMPLOYMENT_DS_BK
SET EMPLOYMENT = ( SELECT eo.EMPLOYMENT
					FROM projlib.TESTING_FI_EMPLOYMENT_DS_BK_NEW eo
					WHERE eo.TOTAL_COUNTS EQ ( SELECT MAX(ei.TOTAL_COUNTS)
										 	   FROM projlib.TESTING_FI_EMPLOYMENT_DS_BK_NEW ei ) )
...
LOAN_HISTORY - Categorical Variable
*******************************/
/* Step 1. Make a copy of projlib.TESTING_FI_EMPLOYMENT_DS_BK */

PROC SQL;

CREATE TABLE projlib.TESTING_FI_LOAN_HISTORY_DS_BK AS
SELECT * FROM projlib.TESTING_FI_EMPLOYMENT_DS_BK;
...
PROC SQL;
SELECT *
FROM projlib.TESTING_FI_LOAN_HISTORY_DS_BK lh
WHERE ( ( lh.LOAN_HISTORY EQ . ) OR
		( lh.LOAN_HISTORY IS NULL) );
QUIT;
...
CREATE TABLE projlib.TESTING_FI_LH_DS_BK_NEW AS
SELECT lh.LOAN_HISTORY AS LOAN_HISTORY, COUNT(*) AS TOTAL_COUNTS
FROM projlib.TESTING_FI_LOAN_HISTORY_DS_BK lh
WHERE ( ( lh.LOAN_HISTORY IS NOT NULL ) OR
		( lh.LOAN_HISTORY NE . ) )
...
SELECT * 
FROM projlib.TESTING_FI_LH_DS_BK_NEW;
QUIT;
...
PROC SQL;
SELECT MAX(lh.TOTAL_COUNTS) AS MAX_COUNTS
FROM projlib.TESTING_FI_LH_DS_BK_NEW lh;
QUIT;
...
PROC SQL;
SELECT lho.LOAN_HISTORY
FROM projlib.TESTING_FI_LH_DS_BK_NEW lho
WHERE lho.TOTAL_COUNTS EQ ( SELECT MAX(lhi.TOTAL_COUNTS)
						   FROM projlib.TESTING_FI_LH_DS_BK_NEW lhi); 
						   /* subquery to find the mode for loan history */
QUIT;
...
TITLE 'Impute the missing values using mode';
PROC SQL;
UPDATE projlib.TESTING_FI_LOAN_HISTORY_DS_BK
SET LOAN_HISTORY = ( SELECT lho.LOAN_HISTORY
					FROM projlib.TESTING_FI_LH_DS_BK_NEW lho
					WHERE lho.TOTAL_COUNTS EQ ( SELECT MAX(lhi.TOTAL_COUNTS)
										 	   FROM projlib.TESTING_FI_LH_DS_BK_NEW lhi ) )
...
/************************************************************************
Imputation for continuous variable on dataset projlib.TESTING_TP061966
*************************************************************************/
/******************************
LOAN_AMOUNT - Continuous Variable
******************************/
...
PROC SQL;
SELECT * 
FROM projlib.TESTING_FI_LOAN_HISTORY_DS_BK la
WHERE ( la.LOAN_AMOUNT EQ . );
QUIT;
...
PROC SQL;
CREATE TABLE projlib.TESTING_FI_LOAN_AMOUNT_DS_BK AS
SELECT * FROM projlib.TESTING_FI_LOAN_HISTORY_DS_BK;
QUIT;

/* Step 4. Impute the missing values using mean for "LOAN_AMOUNT" variable */
TITLE 'Impute the missing values using mean';
PROC STDIZE DATA = projlib.TESTING_FI_LOAN_AMOUNT_DS_BK REPONLY
METHOD = MEAN OUT=projlib.TESTING_FI_LOAN_AMOUNT_DS_BK;
VAR LOAN_AMOUNT;
QUIT;
...
PROC SQL;
SELECT * 
FROM projlib.TESTING_FI_LOAN_AMOUNT_DS_BK ld
WHERE ( ld.LOAN_DURATION EQ . );
QUIT;
...
PROC SQL;
CREATE TABLE projlib.TESTING_FI_LOAN_DURATION_DS_BK AS
SELECT * FROM projlib.TESTING_FI_LOAN_AMOUNT_DS_BK;
QUIT;

/* Step 4. Impute the missing values using mean for "LOAN_DURATION" variable */
TITLE 'Impute the missing values using mean';
PROC STDIZE DATA = projlib.TESTING_FI_LOAN_DURATION_DS_BK REPONLY
METHOD = MEAN OUT=projlib.TESTING_FI_LOAN_DURATION_DS_BK;
VAR LOAN_DURATION;
QUIT;
...
**********************************************************/
PROC LOGISTIC DATA = projlib.TRAINING_FI_LOAN_DURATION_DS_BK OUTMODEL = projlib.TRAINING_DS_LRMODEL;
...
LOAN_HISTORY
LOAN_LOCATION;
OUTPUT OUT = projlib.TRAINING_DS_LRMODEL_OUTPUT P = PRED_PROB;

RUN;

/* Show the predicted probability that was discovered inside the “projlib.TRAINING DS” dataset */

PROC SQL;
SELECT * FROM projlib.TRAINING_DS_LRMODEL_OUTPUT;
QUIT;

/* Show the content of the LR Model that was generated */

PROC SQL;
SELECT * FROM projlib.TRAINING_DS_LRMODEL;
QUIT;

/* Create the Prediction */

PROC LOGISTIC INMODEL = projlib.TRAINING_DS_LRMODEL; /* Passing the Model that has been created */
SCORE DATA = projlib.TESTING_FI_LOAN_DURATION_DS_BK /* Passing the Testing Dataset */
OUT = projlib.TESTING_DS_PREDICTIONS; /* The output location */
QUIT;

/* Display the Prediction */

PROC SQL;
SELECT * FROM projlib.TESTING_DS_PREDICTIONS;
QUIT;
